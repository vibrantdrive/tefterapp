...
<body id="clients">
	...
	>
		<div class="group block">
			...
			<h1>{$lang.title_clients}</h1>
			{if $user.level eq 99}
			<a href="{base_url}clients/add" class="btn add"><span>{$lang.button_add_client}</span></a>
			{/if}
				...
				<div id="filters">
					<form action="#" method="post" class="group" onSubmit="return false;" >
						<div class="group">
							...
							<button type="submit" onclick="{literal}javascript: searchClientClick($('#search').val());{/literal}">{$lang.button_search}</button>
						</div>
						...
						<input type="hidden" name="call_script_table" id="call_script_table" value="clients/table">
					</form>
				</div>
			...
			<ol class="group reset clients-list">
			</ol>
			<!-- data -->
			</div>
			...
			{else}
				{include file='clients/clients_blank.tpl'}
			{/if}
		</div>
	</section>
